...
class Enemy(Object):
    ...
    def __init__(self , x , y):
        ...
        self.enemyes = [pygame.image.load( join ( "assets" , "enemy", f"attack_{i}.png" ) ).convert_alpha() for i in range( 1 , 11 )]
...
class score(Object):
    ...
    def __init__(self , x , y , width , height):
        ...
        self.sprites = [pygame.transform.scale2x(pygame.image.load( join( "assets" , "Menu" , "Levels" , str(i) + ".png" ) ).convert_alpha()).convert_alpha() for i in range(1,51)]
        self.image = self.sprites[SCORE - 1]
        self.mask = pygame.mask.from_surface(self.image)
        self.animation_count = 0
        self.animation_name = "Checkpoint1"
        self.side = pygame.image.load( join( "assets" , "MainCharacters" , "VirtualGuy", "fall.png")).convert_alpha()
...
def main(window):
    ...
    *[Fruits(-block_size *i + 20 , HEIGHT - block_size * 6 - 60 , 32 , 32 , "Bananas") for i in range ( 6 , 8 )]
    ...
    while run:
        ...
        if end.animation_name == "Gate_open" and gate in objects :
            objects.remove(gate)
